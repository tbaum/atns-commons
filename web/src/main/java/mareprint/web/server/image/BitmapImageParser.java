 ...
 */
public class BitmapImageParser implements ImageParser {
// ------------------------------ FIELDS ------------------------------
...
// -------------------------- STATIC METHODS --------------------------
    ...
    public static String getMimeType(byte[] data) {
        final BitmapImageParser imageParser = new BitmapImageParser();
        imageParser.setInput(new ByteArrayInputStream(data));

        return imageParser.getMimeType();
    }
...
// -------------------------- OTHER METHODS --------------------------
    ...
    private boolean checkGif() throws IOException {
        ...
        final byte[] a = new byte[11]; // 4 from the GIF signature + 7 from the global header
        if (read(a) != 11) {
            return false;
        }
    ...
    private boolean checkPnm(final int id) throws IOException {
        ...
        while (true) {
            ...
            if (!hasPixelResolution) { // split "343 966" into width=343, height=966
                ...
                final int spaceIndex1 = s.lastIndexOf(' ');
                if (spaceIndex1 == -1) {
                    return false;
                }
                final String heightString = s.substring(spaceIndex1 + 1);
                try {
    ...
    private String readLine() throws IOException {
        return readLine(new StringBuffer(1000));
    }
    ...
    public float getHeightInch() {
        final int h = height;
        final int ph = physicalHeightDpi;
        if (h > 0 && ph > 0) {
    ...
    public float getWidthCentimeter() {
        return Utils.incToCentimeter(getWidthInch());
    }

    public float getHeightCentimeter() {
        return Utils.incToCentimeter(getHeightInch());
    }
    public float getWidthInch() {
        final int w = width;
        final int pw = physicalWidthDpi;
        if (w > 0 && pw > 0) {
